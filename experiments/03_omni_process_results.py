...
pairs = pd.read_parquet("data/omni/omni_pairs.parquet")
pairs.set_index(["full_id"], inplace=True)

pair_results_o3 = pd.read_json(
    "results/omni/omni_pairs_o3_results.jsonl",
    lines=True,
    dtype={"response": str, 'custom_id': str},
)
pair_results_gemini = pd.read_json(
    "results/omni/omni_pairs_gemini_results.jsonl",
    lines=True,
...
pairs.to_parquet(
    "results/omni/omni_pairs_results.parquet",
    index=False,
)
